...
describe Bosh::Agent::Platform::Ubuntu::Network do
  ...
  context 'AWS' do
    ...
    context 'when ifup version is 0.6' do
      it 'should configure dhcp with dns server prepended' do
        ...
        expect(network_wrapper).to receive(:sh).with('ifup --version', :on_error => :return).twice
                                   .and_return(Bosh::Exec::Result.new('ifup --version', 'ifup version 0.6.0', 0))
        expect(network_wrapper).to receive(:sh).with('ifdown -a --exclude=lo', :on_error => :return)
        expect(network_wrapper).to receive(:sh).with('ifup -a --exclude=lo', :on_error => :return)

        network_wrapper.setup_networking
      end
    end

    context 'when ifup version is 0.7' do
      it 'should configure dhcp with dns server prepended' do
        expect(Bosh::Agent::Util).to receive(:update_file) do |contents, file|
          expect(contents).to match /^prepend domain-name-servers 5\.6\.7\.8;\nprepend domain-name-servers 1\.2\.3\.4;$/
          expect(file).to eq('/etc/dhcp/dhclient.conf')
        ...
        expect(network_wrapper).to receive(:sh).with('pkill dhclient', :on_error => :return)
        expect(network_wrapper).to receive(:sh).with('ifup --version', :on_error => :return).twice
                                   .and_return(Bosh::Exec::Result.new('ifup --version', 'ifup version 0.7.47', 0))
        expect(network_wrapper).to receive(:sh).with('ifdown -a --no-loopback', :on_error => :return)
        expect(network_wrapper).to receive(:sh).with('ifup -a --no-loopback', :on_error => :return)
